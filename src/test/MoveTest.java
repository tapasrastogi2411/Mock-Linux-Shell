...
public class MoveTest {
  ...
  /**
   * Move variable so that we can test its methods
   */
  Move mv;

  /**
   * MakeDirectory variable so that we can test the Copy methods
   */
  MakeDirectory mkdir;

  /**
   * Tree variable to check the file System structure after move command is executed
   */
  Tree tree;
  ...
  public void testExecuteCommand(){
    ...
    String[] arguments = {"mkdir",  "a",  "a/b", "c"};
    mkdir.executeCommand(arguments, false);

    String[] argument1 = {"mv", "/a", "/c"};
    mv.executeCommand(argument1);

    String actual = tree.executeCommand(argument1, false);
